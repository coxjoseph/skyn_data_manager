...
from SDM.User_Interface.Utils.filename_tools import (extract_subid, extract_dataset_identifier,
                                                     get_used_dataset_identifiers, matches_filename_convention,
                                                     update_filename_parsing_indices, directory_analysis_ready,
                                                     processor_data_ready)
...
def verify_metadata(filenames, metadata):
    ...
    for file in filenames:
        ...
        matching_metadata = metadata[
            (metadata['SubID'] == int(subid)) & (metadata['Dataset_Identifier'] == int(dataset_id))]
        ...
        if len(matching_metadata) == 0:
            ...
            messagebox.showerror('SDM Error',
                                 f'Duplicate episode IDs ({", ".join(episode_ids)}) for '
                                 f'SubID: {subid}, Dataset ID: {dataset_id} ')
            return False
...
def verify_filename(sdm_interface, filename, directory):
    ...
    if filename_valid:
        confirmed_by_user = messagebox.askyesno('SDM',
                                                f'Is data set info correct?\n'
                                                f'SubID = {subid}\n'
    ...
    if filename_valid and confirmed_by_user:
        ...
        rename_files = RenameFilesWindow(sdm_interface, directory, sdm_interface.filenames)
        rename_files.grab_set()
        sdm_interface.wait_window(rename_files)
        return False
...
def verify_processor(processor_file, min_datasets_required):
    try:
        with open(processor_file.name, "rb") as pickle_in:
            skyn_processor = pickle.load(pickle_in)

        if not processor_data_ready(skyn_processor, min_datasets_required):
            messagebox.showerror('SDM Error', 'This file does not have processed Skyn data.')
            return None

        if not all(occasion.condition in ['Alc', 'Non', 'Unk'] for occasion in skyn_processor.occasions):
            messagebox.showerror(
                'SDM Error',
                'Datasets are not assigned valid "Condition" labels in Metadata. '
                'Labels must be: Alc, Non, or Unk.'
            )
            return None
        ...
        error_message = f"Error loading processor file: {e}"
        print(error_message)
        messagebox.showerror('SDM Error', error_message)
        return None

    except AttributeError as e:
        error_message = f"Invalid file structure or attributes: {e}"
        print(error_message)
        messagebox.showerror('SDM Error', error_message)
        return None
...
def create_processor(settings_window, sdm_interface):
    ...
    cohort_name = sdm_interface.cohortNameEntry.get() if data_format not in ['Processor', 'Test'] else (
        sdm_interface.previous_processor.cohort_name) if data_format == 'Processor' else 'Test'
    ...
    if data_format != 'Processor':
        ...
        if data_format == 'Test':
            ...
            if sdm_interface.metadata != '':
                matching_metadata = (sdm_interface.metadata_df.loc[
                    ((sdm_interface.metadata_df['SubID'] == str(subid)) |
                     (sdm_interface.metadata_df['SubID'] == int(subid))) &
                    ((sdm_interface.metadata_df['Dataset_Identifier'] == str(dataset_identifier)) |
                     (sdm_interface.metadata_df['Dataset_Identifier'] == int(dataset_identifier)))].
            ...
            if len(matching_metadata) > 1:
                ...
                while not episode_identifier_registered:
                    episode_identifier = simpledialog.askinteger(
                        "Metadata indicates that multiple episodes were identified within the chosen dataset",
                        f'Please enter one of the available Episode IDs: '
                        f'{" ,".join([str(pid) for pid in matching_episode_ids])}')
            ...
            if not episode_identifier:
                messagebox.showerror('SDM Error',
                                     f'Episode Identifier could not be loaded from metadata. '
                                     f'Please check metadata: {sdm_interface.metadata}')
            else:
        ...
        if sdm_interface.selected_programs['Train+Test']:
            ...
            for name in [name for name in existing_model_names if name in pending_model_names]:
                remove_model_from_queue = messagebox.askyesno('SDM',
                                                              f'Model [{name}] has already been trained; '
                                                              f'would you like to retrain and overwrite it?\n'
                                                              f'To overwrite, click YES. Otherwise, click NO.')
                if remove_model_from_queue:
                    settings_window.TrainSettingsFrame.unclick_model(name)
                else:
                    model_to_overwrite = [model for model in sdm_processor.models if model.model_name == name][0]
                    sdm_processor.models.remove(model_to_overwrite)
...
def load_settings(settings_window):
    if settings_window.sdm_interface.data_loading_method != 'Processor':
        return {
            ...
            'disable_crop_start':
                settings_window.CropSettingsFrame.disableCropStart.get() if
                settings_window.CropSettingsFrame.crop_start_timestamps_available else 1,
            'disable_crop_end':
                settings_window.CropSettingsFrame.disableCropEnd.get() if
                settings_window.CropSettingsFrame.crop_end_timestamps_available else 1,
            'min_duration_active':
            ...
            'training_features':
                [key for key, val in settings_window.TrainSettingsFrame.predictor_checkboxes.items() if val.get()] if
                settings_window.sdm_interface.selected_programs[
                    'Train+Test'] else settings_window.TrainSettingsFrame.predictor_options,
        }
    else:
        return {
            'models':
            ...
            'training_features':
                [key for key, val in settings_window.TrainSettingsFrame.predictor_checkboxes.items() if val.get()] if
                settings_window.sdm_interface.selected_programs[
                    'Train+Test'] else settings_window.sdm_interface.previous_processor.training_features
        }
