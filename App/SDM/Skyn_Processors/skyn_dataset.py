...
from SDM.Configuration.configuration import configure_timestamps, get_full_identifier, includes_multiple_device_ids, \
    get_cohort_full_identifiers, get_closest_index_with_timestamp, get_time_elapsed, update_column_names, \
    rename_tac_column, get_sampling_rate, reduce_sampling_rate, remove_junk_columns
from SDM.Configuration.file_management import load_default_model
from SDM.Crop.crop import get_start_date_time, crop_device_off_end, crop_device_off_start, get_utc_offset
from SDM.Feature_Engineering.tac_features import get_discrete_curve_count, get_tac_directional_alterations, \
    get_tac_directional_alteration_percent, get_avg_tac_diff, get_mean_stdev_sem, get_value_proportion, \
    get_curve_auc_per_hour, get_curve_auc, get_curve_duration, get_fall_rate, get_rise_rate, get_fall_duration, \
    get_rise_duration, get_baseline_mean_stdev, get_auc, get_peak, get_peak_index, get_curve_threshold, \
    get_fall_completion, get_rise_completion
from SDM.Signal_Processing.remove_outliers import replace_extreme_values, impute_artifacts
from SDM.Signal_Processing.smooth_signal import smooth_signals
from SDM.Visualization.plotting import plot_cleaning_comparison, plot_TAC_curve, plot_overlaid_TAC_curves, \
    plot_device_removal, plot_cropping, plot_smoothed_curve, plot_column, plot_tac_and_temp
...
# TODO: refactor and rewrite this entire class
class SkynDataset:
    def __init__(self, path, data_out_folder, graphs_out_folder, subid, dataset_identifier, episode_identifier='e1',
        ...
        self.event_timestamps = self.get_event_timestamps(metadata_path)
    ...
    def process_with_default_settings(self, make_plots=False, export=True):
        ...
        # This should raise errors if something goes wrong - but for now will just log and ignore
        if make_plots:
            try:
    ...
    def clean_tac_signal(self):
        ...
        if slope < -3 and df.loc[:12, 'TAC'].min() < 10:
            df.loc[:12, 'TAC'] = 0
            df['TAC_sloped_start_reassigned'] = [0 for _ in range(12)] + df.loc[12:, 'TAC'].tolist()
            df['sloped_start'] = [1 for _ in range(12)] + [0 for _ in range(12, len(df))]
        else:
        ...
        if (len(df) * self.sampling_rate) > 60:
            df = impute_tac_in_gaps(df, 'TAC', 'Duration_Hrs', self.sampling_rate,
                                    self.gap_fill_max_duration)
        if 'gap_imputed' not in self.dataset.columns:
            df['gap_imputed'] = [0 for _ in range(len(df))]
        ...
        if (len(self.dataset) > 30) and (max(self.dataset['TAC']) - min(self.dataset['TAC']) > 5):
            ...
            self.dataset['extreme_outlier'] = [1 if i in extreme_outliers else 0 for i in range(len(self.dataset))]
        else:
            extreme_outliers = []
            self.dataset['extreme_outlier'] = [0 for _ in range(len(self.dataset))]
        ...
        if (len(self.dataset) > 30) and (max(df['TAC']) - min(df['TAC']) > 5) and (self.valid_occasion == 1):
            ...
            df['major_outlier'] = [(1 if i in major_outliers else 0) for i in range(len(df))]
            df['minor_outlier'] = [(1 if i in minor_outliers else 0) for i in range(len(df))]
    ...
    def crop_dataset_with_timestamps(self):
        self.dataset, cropped_plot_path = self.crop_using_timestamps(self.dataset)
    ...
    def detect_device_removal(self):
        """Will detect device removal based on Temp value or trained algorithm (which assesses recent temp and motion
        changes), then impute regions where device was removed or unstable.
        """
        counts = 0
        ...
        if 'Temp' in self.device_removal_detection_method:
            ...
            try:
                ...
                print(f"Error in detecting device removal: {e}")
                temp_cutoff = 27
    ...
    def set_curve_threshold(self, dataset_version='CLN', validate=False):
        ...
        if tac_range < 2:
            ...
            while not curve_threshold_verified:
                ...
                if (curve_threshold < (tac_range / peak_to_floor_required_ratio) + floor) or (tac_range < 3) or (
                    ...
                    if curve_threshold > peak or curve_threshold < floor:
                        curve_threshold = floor + (tac_range * 0.8)
            ...
            if self.dataset['Duration_Hrs'].max() < 1 or (drop_count * self.sampling_rate) > (len(self.dataset) / 3):
                ...
                peak = get_peak(self.dataset, tac_variable)
                floor = self.dataset[tac_variable].min()
                # Previously set to the value of get_curve_threshold - but overwritten. TODO: check
                curve_threshold = (peak + floor) / 2
        else:
            curve_threshold = get_curve_threshold(self.dataset, tac_variable, self.baseline_range)
    ...
    def get_episode_features(self, dataset_version='RAW'):
        ...
        (self.stats[f'auc_total_{dataset_version}'],
         self.stats[f'auc_per_hour_{dataset_version}']) = get_auc(df, tac_variable, self.time_elapsed_column)
        ...
        (self.stats[f'fall_duration_{dataset_version}'],
         curve_ends_index,
         curve_fall_threshold) = get_fall_duration(df, tac_variable, self.time_elapsed_column, peak_index,
                                                   self.stats[f'curve_threshold_{dataset_version}'])
        ...
        self.stats[f'completed_curve_count_{dataset_version}'] = (
            get_discrete_curve_count(df, tac_variable, self.sampling_rate,
                                     self.stats[f'curve_threshold_{dataset_version}'], min_curve_duration_hours=0.5))
    ...
    def get_row_features(self):
        ...
        if len(list(self.event_timestamps.keys())) > 0:
            ...
            self.dataset['device_on'] = ['unk' for _ in range(len(self.dataset))]
    ...
    def plot_device_removal(self):
        ...
        if plot_path not in self.complex_plot_paths and plot_path is not None:
            self.complex_plot_paths.append(plot_path)
    ...
    def plot_tac_curves(self, additional_groupings=None):
        if additional_groupings is None:
    ...
    def label_device_removed_with_timestamps(self):
        ...
        for timestamp_label, timestamp in self.event_timestamps.items():
            ...
            if timestamp_index:
                # if timestamp indicates device was taken off, then prior data corresponds to device on
                if "off" in timestamp_label.lower():
                    labels.extend([1 for _ in range(prior_timestamp_index, timestamp_index)])
                elif "on" in timestamp_label.lower():
                    labels.extend([0 for _ in range(prior_timestamp_index, timestamp_index)])
                else:
                    labels.extend(['unk' for _ in range(prior_timestamp_index, timestamp_index)])
                ...
                if timestamp_label == list(self.event_timestamps.keys())[len(list(self.event_timestamps.keys())) - 1]:
                    if "off" in timestamp_label.lower():
                        labels.extend([0 for _ in range(timestamp_index, len(self.dataset))])
                    elif "on" in timestamp_label.lower():
                        labels.extend([1 for _ in range(timestamp_index, len(self.dataset))])
                    else:
                        labels.extend(['unk' for _ in range(timestamp_index, len(self.dataset))])

        self.dataset['device_on'] = labels

    def make_prediction(self, models):
        # Todo: removed export parameter (unused) - if necessary add back
        predictions = {}
        for model in models:
            try:
                ...
                if model.outcome == 'condition':
                    ...
                    if self.condition != 'Unk':
                        ...
                        predictions[
                            model.model_name + '_self_report_or_ground_truth_confirmed'] = ('Cannot confirm - no '
                                                                                            'self-report or ground '
                                                                                            'truth provided for '
                                                                                            'condition')
                    ...
                    if self.binge != "None" and self.binge != "Unk":
                        ...
                        predictions[
                            model.model_name + '_self_report_or_ground_truth_confirmed'] = ('Cannot confirm - Binge '
                                                                                            'prediction may not be '
                                                                                            'relevant. '
                                                                                            'Self-report/ground truth '
                                                                                            'is non-alcohol episode.')
                    else:
                        predictions[
                            model.model_name + '_self_report_or_ground_truth_confirmed'] = ('Cannot confirm - no '
                                                                                            'self-report or ground '
                                                                                            'truth determined for '
                                                                                            'binge.')
                else:
                    predictions[model.model_name] = prediction
                    predictions[
                        model.model_name + '_self_report_or_ground_truth_confirmed'] = ('Cannot confirm - model is '
                                                                                        'exploratory and has not been'
                                                                                        ' implemented into SDM.')
    ...
    def crop_using_timestamps(self, dataset):
        """Crop dataset using timestamps file (see Resource folder for example). If Crop End Timestamps are also
        provided, data after this timestamp will be removed. Otherwise, data after the default max duration will be
        removed.
        """
        filter_condition = (
